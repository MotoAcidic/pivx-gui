...
#include "qt/yieldstakingwallet/dashboardwidget.h"
#include "qt/yieldstakingwallet/forms/ui_dashboardwidget.h"
#include "qt/yieldstakingwallet/sendconfirmdialog.h"
#include "qt/yieldstakingwallet/txrow.h"
...
DashboardWidget::DashboardWidget(YieldSakingWalletGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareYsw, "square-chart-ysw");
    setCssProperty(ui->labelSquarezYsw, "square-chart-zysw");
    setCssProperty(ui->labelYsw, "text-chart-ysw");
    setCssProperty(ui->labelZysw, "text-chart-zysw");
    ...
    setCssProperty(ui->labelAmountYsw, "text-stake-ysw-disable");
    setCssProperty(ui->labelAmountZysw, "text-stake-zysw-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &YieldSakingWalletGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isYsw = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZYSW;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair YSW, zYSW
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            ysw = (pair.first != 0) ? pair.first / 100000000 : 0;
            zysw = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalYsw += pair.first;
            chartData->totalZysw += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesYsw.append(ysw);
        chartData->valueszYsw.append(zysw);

        int max = std::max(ysw, zysw);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->append(chartData->valuesYsw);
    set1->append(chartData->valueszYsw);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalYsw > 0 || chartData->totalZysw > 0) {
        setCssProperty(ui->labelAmountYsw, "text-stake-ysw");
        setCssProperty(ui->labelAmountZysw, "text-stake-zysw");
    } else {
        setCssProperty(ui->labelAmountYsw, "text-stake-ysw-disable");
        setCssProperty(ui->labelAmountZysw, "text-stake-zysw-disable");
    }
    forceUpdateStyle({ui->labelAmountYsw, ui->labelAmountZysw});
    ui->labelAmountYsw->setText(GUIUtil::formatBalance(chartData->totalYsw, nDisplayUnit));
    ui->labelAmountZysw->setText(GUIUtil::formatBalance(chartData->totalZysw, nDisplayUnit, true));
