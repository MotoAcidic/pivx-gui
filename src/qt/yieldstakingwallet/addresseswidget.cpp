...
#include "qt/yieldstakingwallet/forms/ui_addresseswidget.h"
#include "qt/yieldstakingwallet/addresslabelrow.h"
#include "qt/yieldstakingwallet/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(YieldSakingWalletGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto yswAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!Standard::IsValidDestination(yswAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(yswAdd);
        if (walletModel->updateAddressBookLabels(yswAdd, label.toUtf8().constData(),
