...
#include "qt/yieldstakingwallet/forms/ui_snackbar.h"
#include "qt/yieldstakingwallet/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(YieldSakingWalletGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &YieldSakingWalletGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
