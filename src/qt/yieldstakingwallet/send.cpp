...
#include "qt/yieldstakingwallet/forms/ui_send.h"
#include "qt/yieldstakingwallet/addnewcontactdialog.h"
#include "qt/yieldstakingwallet/qtutils.h"
#include "qt/yieldstakingwallet/sendchangeaddressdialog.h"
#include "qt/yieldstakingwallet/optionbutton.h"
...
SendWidget::SendWidget(YieldSakingWalletGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-ysw");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onYSWSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onYSWSelected(false);});
...
void SendWidget::showHideCheckBoxDelegations()
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent YSWs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (walletModel && availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res.result) {
                inform(tr("Error generating address to shield YSWs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent YSWs to shield."));
    }
}
...
void SendWidget::onYSWSelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto yswAdd = Standard::DecodeDestination(address.toStdString(), isStakingAddr);

        if (!Standard::IsValidDestination(yswAdd) || isStakingAddr) {
            inform(tr("Invalid address"));
            return;
        }

        if (walletModel->isMine(yswAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(yswAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
