...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select YSW Outputs to Spend" :
            "Select Shielded YSW to Spend");
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show YieldStakingWallet address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {
