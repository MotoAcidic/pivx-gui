...
#include "qt/yieldstakingwallet/splash.h"
#include "qt/yieldstakingwallet/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("yieldstakingwallet-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in yieldstakingwallet.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in yieldstakingwallet.qrc)
...
/** Class encapsulating YieldStakingWallet Core startup and shutdown.
...
/** Main YieldStakingWallet application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (YieldSakingWalletGUI)
...
#include "yieldstakingwallet.moc"
...
{
    window = new YieldSakingWalletGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &YieldSakingWalletGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &YieldSakingWalletGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(YieldSakingWalletGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(YieldSakingWalletGUI::DEFAULT_WALLET);
        ...
        // YieldStakingWallet: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &YieldSakingWalletGUI::handlePaymentRequest);
        connect(window, &YieldSakingWalletGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. YieldStakingWallet can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    ...
    /// 6. Determine availability of data directory and parse yieldstakingwallet.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("YieldStakingWallet Core"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("YieldStakingWallet Core"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // yieldstakingwallet: links repeatedly have their payment requests routed to this process:
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("YieldStakingWallet Core didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
