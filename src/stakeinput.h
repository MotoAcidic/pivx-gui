...
#ifndef YieldSakingWallet_STAKEINPUT_H
#define YieldSakingWallet_STAKEINPUT_H
...
public:
    CYswStake(const CTxOut& _from, const COutPoint& _outPointFrom, const CBlockIndex* _pindexFrom) :
            CStakeInput(_pindexFrom), outputFrom(_from), outpointFrom(_outPointFrom) {}

    static CYswStake* NewYswStake(const CTxIn& txin);
    ...
    bool IsZYSW() const override { return false; }
    bool ContextCheck(int nHeight, uint32_t nTime) override;
};


#endif //YieldSakingWallet_STAKEINPUT_H
