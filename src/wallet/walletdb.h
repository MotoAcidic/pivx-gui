...
public:
    ...
    bool WriteZYSWSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZYSWSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZYSWSeed_deprecated(uint256& seed);
    bool EraseZYSWSeed();
    bool EraseZYSWSeed_deprecated();

    bool WriteZYSWCount(const uint32_t& nCount);
    bool ReadZYSWCount(uint32_t& nCount);
