...
function install_params {
    ...
    if ! [ -f "$filename" ]
    then
        filename="share/yieldstakingwallet/$filename"
    fi
...
function main() {
    ...
    cat <<EOF
YieldStakingWallet - install-params.sh

This script will install the YieldStakingWallet zkSNARK parameters and verify their
