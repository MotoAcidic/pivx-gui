YieldStakingWallet Core version *v5.0.0* is now available from:  <https://github.com/yieldstakingwallet-project/yieldstakingwallet/releases>
...
YieldStakingWallet Core v5.0.0 is a mandatory update for all users. This release contains a protocol upgrade (to `70920`), new consensus rules and improvements that are not backwards compatible with older versions. Users will need to update their clients before enforcement of this update goes into effect.
...
For the other packages, the user must save the param files in the proper location, before being able to run YieldStakingWallet v5.0.0:
- macOS/Linux `tar.gz` tarballs include a bash script (`install-params.sh`) to copy the parameters in the appropriate location.
- Windows `.zip` users need to manually copy the files from the `share/yieldstakingwallet` folder to the `%APPDATA%\YieldSakingWalletParams` directory.
...
#### Overview
Users will be able to protect their financial information by sending and receiving YSWs privately, as well as sending and receiving encrypted messages attached to the shield transactions.
More visual information about the protocol can be found at https://yieldstakingwallet.org .
...
* Top bar showing shield YSW balances.
...
* New derivation path for Sapling keys: Shield addresses are derived from the same wallet master seed used for the deterministic derivation of all other keys. The same seed, therefore, can be used to restore both transparent and Shield addresses, recovering both transparent and Shield YSW balance.
...
See the release notes from [v4.2.0](https://github.com/YieldStakingWallet-Project/YieldStakingWallet/blob/master/doc/release-notes/release-notes-4.2.0.md#label-and-account-apis-for-wallet) for a full description of the changes from the 'account' API to the 'label' API.
...
| `getsupplyinfo` | Returns detailed YSW supply information | No |
...
* `viewshieldtransaction`
  ...
  Result:
  {
    ...
    "fee"  : x.xxx,             (numeric) The transaction fee in YSW
    "spends" : [
      {
...
* `getinfo`
  The `zerocoinbalance` JSON return entry and the `zYSWsupply` return JSON object have been removed due to zerocoin deprecation.
  The `balance` JSON return entry is now all inclusive (the sum of all transparent and shield YSW)
  The `moneysupply` JSON return entry is the total YSW supply (the sum of all transparent and shield YSW)
...
| `getaccountaddress` | Returned the current YieldStakingWallet address associated with an internal account | Account system removal |
| `setaccount` | Associate a YieldStakingWallet address with an internal account name | Account system removal |
| `getaccount` | Returned the internal account associated with a given YieldStakingWallet address | Account system removal |
| `getaddressesbyaccount` | Returned a list of YieldStakingWallet addresses associated with an internal account | Account system removal |
...
| `sendfrom` | Send an amount from an internal account to a YieldStakingWallet address | Account system removal |
...
| `multisend` | Configure sending a portion of stake rewards to a YieldStakingWallet address | Pending a full rewrite |
| `getzerocoinbalance` | Returned the wallet's total zYSW balance | Zerocoin deprecation |
| `listmintedzerocoins` | List all zYSW mints in the wallet | Zerocoin deprecation |
| `listzerocoinamounts` | List information about your zerocoin amounts | Zerocoin deprecation |
| `listspentzerocoins` | List all the spent zYSW mints in the wallet | Zerocoin deprecation |
| `mintzerocoin` | Mint the specified zYSW amount | Zerocoin deprecation |
| `spendzerocoin` | Spend zYSW to a YSW address | Zerocoin deprecation |
| `spendzerocoinmints` | Spend specific zYSW mints to a YSW address | Zerocoin deprecation |
...
| `reconsiderzerocoins` | Check archived zYSWs to see if any mints were added to the chain | Zerocoin deprecation |
| `setzyswseed` | Set the wallet's deterministic zYSW seed to a specific value | Zerocoin deprecation |
| `getzyswseed` | Return the current deterministic zYSW seed | Zerocoin deprecation |
| `generatemintlist` | Show mints that are derived from the deterministic zYSW seed | Zerocoin deprecation |
| `dzyswstate` | Show the current state of the mintpool of the wallet | Zerocoin deprecation |
| `searchdzysw` | Do a search for deterministically generated zYSW that have not yet added to the wallet | Zerocoin deprecation |
...
- #2041 `a15dc3417d` [Build] CMake yswd and unit tests: fixing missing MacOS framework. (furszy)
