...
- [Development guidelines](#development-guidelines)
    - [General YieldStakingWallet Core](#general-yieldstakingwallet-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
YieldStakingWallet Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
YieldStakingWallet Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/yieldstakingwallet-project/yieldstakingwallet/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_yieldstakingwallet
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_yieldstakingwallet --log_level=test_suite
$ valgrind -v --leak-check=full src/yieldstakingwalletd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_yieldstakingwallet.coverage/index.html`.
```
...
- BitcoinMiner : Generates YSWs (if wallet is enabled).
...
However, in open source software such as YieldStakingWallet Core, where everyone uses
...
pay attention to for reviewers of YieldStakingWallet Core code.

General YieldStakingWallet Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: YieldStakingWallet Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against YieldStakingWallet Core so that they can be integrated
...
file descriptors it will cause problems with YieldStakingWallet's `select()` loop, because
...
```bash
$ lsof -p $(pidof yieldstakingwalletd) |\
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:YieldStakingWallet-Project/YieldStakingWallet.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `yieldstakingwallet-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `yieldstakingwallet-cli` and the GUI debug console use this table to determine how to
