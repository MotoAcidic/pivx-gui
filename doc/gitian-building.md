...
*Setup instructions for a gitian build of YieldStakingWallet Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the YieldStakingWallet
...
to the YieldStakingWallet GitHub release page.
...
- [Building YieldStakingWallet Core](#building-yieldstakingwallet-core)
...
Building YieldStakingWallet Core
--------------------
...
```bash
gpg --output ${VERSION}-linux/${NAME}/yieldstakingwallet-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/yieldstakingwallet-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/yieldstakingwallet-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/yieldstakingwallet-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/yieldstakingwallet-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/yieldstakingwallet-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/yieldstakingwallet-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@yieldstakingwallet.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/yieldstakingwallet-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/yieldstakingwallet-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/yieldstakingwallet-osx-*-build.assert
```
