YieldStakingWallet Core version 2.3.0 is now available from:
...
YieldStakingWallet Core is extensively tested on multiple operating systems using the Linux kernel, macOS 10.8+, and Windows Vista and later.
...
[
  {
    ...
    winner: {
        address: addr,    (string) YieldStakingWallet MN Address,
...
[
  {
    ...
    "addr": addr,      (string) Masternode YieldStakingWallet address
...
yieldstakingwalletd can now (optionally) asynchronously notify clients through a ZMQ-based PUB socket of the arrival of new transactions and blocks. This feature requires installation of the ZMQ C API library 4.x and configuring its use through the command line or configuration file. Please see [docs/zmq.md](/doc/zmq.md) for details of operation.
...
YieldStakingWallet Core now enables bloom filters by default to support SPV clients like mobile wallets. This feature can be disabled by using the `-peerbloomfilters` option on startup.
...
- #195 `aee05fe` [ZMQ] ZMQ integration for YieldStakingWallet (Mrs-X)
...
### Tests and QA
- #191 `3a778c3` [Tests] Fix the unit test suite for use with YieldStakingWallet (Fuzzbawls)
